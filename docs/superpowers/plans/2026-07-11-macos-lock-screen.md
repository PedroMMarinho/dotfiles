...
- **Verification is qmllint + manual runtime.** A session-lock UI cannot be unit-tested; there is no headless harness. Every code step is gated by `qmllint -I /usr/lib/qt6/qml <file>` (must exit 0) followed by a manual runtime check with the escape hatch below.
